...
import { MAJOR_DEAL_OPTIONS } from '../../data/dictionaries';
...
export const GuarantorTab: React.FC<GuarantorTabProps> = ({ entity }) => {
  const deal = useDealStore();

  return (
    <div className="tab-content">
      {/* Блок 1: Информация о поручителе — #7: добавлены 1.12, 1.13; #20: editable Тип клиента */}
      <BlockSection title="1. Информация о поручителе">
        <div className="form-grid">
          ...
          />
          <FormField label="1.3 Роль в сделке" value="Поручитель" />
      ...
      {/* Блок 2: Федресурс */}
      <BlockSection title="2. Сведения о договорах лизинга (Федресурс)">
        <DataTable
          columns={[
      ...
      {/* Блок 3: Кредитные обязательства */}
      <BlockSection title="3. Информация по текущим кредитным обязательствам в Банке">
        <DataTable
          columns={[
            { key: 'contractNumber', header: '№ договора' },
      ...
      {/* Блок 4: Проверка параметров */}
      <BlockSection title="4. Проверка поручителя на соответствие требованиям паспорта продукта">
        <ParameterCheckTable
      ...
      {/* Блок 4.1: Результаты */}
      <BlockSection title="4.1. Результаты проверки на соответствие требованиям паспорта продукта">
      ...
      {/* Блок 6: Финансовое заключение */}
      <BlockSection title="6. Финансовое заключение по поручителю">
        <div style={{ padding: '12px 0', color: '#64748b', fontStyle: 'italic', fontSize: 12 }}>
          Раздел финансового заключения по поручителю.
          <br />
          <span style={{ fontSize: 11 }}>В прототипе: заглушка.</span>
        </div>
      </BlockSection>
    </div>
  );
